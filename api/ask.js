...
Deprem sonrası ilk 72 saat; arama-kurtarma, triyaj, barınma, temiz su, gıda ve lojistik açısından en kritik dönemdir.

Öncelikli aksiyonlar:
1. Güvenli alan oluşturulmalı.
2. Yaralılar triyaj sistemiyle sınıflandırılmalı.
3. Enkaz, gaz kaçağı, yangın ve yol kapanması ayrı raporlanmalı.
4. Su, gıda, battaniye ve tıbbi malzeme ihtiyacı kaydedilmeli.
5. Kritik bölgeler haritada işaretlenmeli.

ARZ önerisi:
Saha bildirimi, harita, lojistik, klinik destek ve halk sağlığı modülleri birlikte kullanılmalıdır.`;
    }

    else if (q.includes("sel") || q.includes("taşkın")) {
      answer = `ARZ Local Brain Analizi – Sel / Taşkın

Sel durumunda temiz suya erişim, kanalizasyon karışması, elektrik tehlikesi ve yol kapanması temel risklerdir.
...
4. Yaşlı, çocuk, gebe ve kronik hastalar öncelikli tahliye edilmeli.

Halk sağlığı notu:
Sel sonrası ishal, cilt enfeksiyonları ve hijyen sorunları açısından takip yapılmalıdır.`;
    }

    else if (q.includes("yangın")) {
      answer = `ARZ Local Brain Analizi – Yangın

Yangında rüzgar yönü, sıcaklık, tahliye koridorları ve yerleşim yerine yakınlık kritik önemdedir.

Öncelikli aksiyonlar:
1. Rüzgar yönüne göre tahliye planı yapılmalı.
2. Duman maruziyeti olanlar sağlık ekiplerine yönlendirilmeli.
3. Yanık vakaları klinik destek modülüne kaydedilmeli.
4. Lojistikte su, maske, serum ve yanık setleri önceliklendirilmelidir.

ARZ önerisi:
Yangın bölgesi haritada turuncu/kırmızı riskle gösterilmeli, gönüllüler sadece güvenli alanlarda görevlendirilmelidir.`;
    }
...
ARZ önerisi:
Klinik destek modülünde nabız, tansiyon, SpO2, ateş, kronik hastalık ve ilaç bilgisi girilmelidir.
...
ARZ önerisi:
Halk sağlığı panelinde su güvenliği, enfeksiyon riski, aşılama oranı ve sağlık hizmetlerinde normalleşme oranı düzenli takip edilmelidir.`;
    }
...
ARZ önerisi:
Raporlar karar destek panelinde kullanılmalı ve olay sonrası değerlendirme için saklanmalıdır.`;
    }
